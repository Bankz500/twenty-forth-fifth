...
class BankingDetailsGenerator {
    ...
    setupFirebase() {
        try {
            // Initialize Firebase
            // Prefer Vite env-injected config (module build), fall back to runtime window config if present.
            const firebaseConfig =
                (typeof window !== 'undefined' && window.__FIREBASE_CONFIG__ && window.__FIREBASE_CONFIG__.apiKey)
            ...
            if (!firebaseConfig || !firebaseConfig.apiKey) {
                throw new Error('Missing Firebase config. Set Vercel env vars (VITE_FIREBASE_*) or load firebase-runtime-config.js before this script.');
            }
