import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css             */import{b as w,q as y,h as T,a as L,w as C,k as $}from"./firebase-client-X0D09DJ_.js";import{onAuthStateChanged as A}from"https://www.gstatic.com/firebasejs/9.6.1/firebase-auth.js";import"./firebase-config-LdbrzKVt.js";function v(l){const m=document.getElementById("auth-nav-button"),a=document.getElementById("auth-nav-button-mobile");!m||!a||(l?(m.innerHTML=`
            ...
            `))}function k(){v(!1),A(w,l=>v(!!l))}window.addEventListener("site:fragments-loaded",k,{once:!0});try{let m=function(o){const d=document.getElementById("testimonials-grid");if(o.length===0){d.innerHTML=`
                    ...
                    `;return}d.innerHTML=o.map((r,c)=>{const p="⭐".repeat(r.rating||0),h=r.createdAt.toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}),u=Number.isFinite(r.likeCount)?`<span class="text-xs text-gray-500 flex items-center gap-1">
                            <i class="fas fa-thumbs-up text-blue-500"></i>
                            <span class="font-medium">${r.likeCount}</span>
                          </span>`:"";let n=r.reviewText||"";n.length>180&&(n=n.substring(0,180)+"...");const g=["from-blue-50 to-indigo-50","from-purple-50 to-pink-50","from-blue-50 to-emerald-50"],f=["border-green-200","border-purple-200","border-green-200"],t=["text-green-600","text-purple-600","text-green-600"],e=g[c%g.length],s=f[c%f.length],i=t[c%t.length];return`
                    ...
                    `}).join("")},a=function(o){const d=document.createElement("div");return d.textContent=o,d.innerHTML};async function l(){try{let p=function(t){const e=(t||"").toLowerCase().trim().replace(/\s+/g," ");return o.some(s=>{const i=s.toLowerCase().trim();return e.includes(i)||i.includes(e)})},h=function(t){const e=(t||"").toLowerCase().trim().replace(/\s+/g," ");return e.includes("point")&&e.includes("speed")?"point speed":e.includes("invest")&&e.includes("wise")?"invest wise":e.includes("blue")&&(e.includes("line")||e.includes("gadget"))?"blueline gadgets":null};const o=["point speed","pointspeed","point-speed","invest wise","investwise","invest-wise","blueline gadgets","blueline","blue line gadgets","blue-line gadgets"],d=y(T(L,"testimonials"),C("published","==",!0)),r=await $(d),c=[];r.forEach(t=>{var s;const e=t.data();c.push({id:t.id,authorName:e.authorName||"",company:e.company||"",country:e.country||"",rating:e.rating||0,reviewText:e.reviewText||"",verified:e.verified||!1,authorImage:e.authorImage||"",likeCount:e.likeCount!==void 0?e.likeCount:null,createdAt:(s=e.createdAt)!=null&&s.toDate?e.createdAt.toDate():e.createdAt?new Date(e.createdAt):new Date})});const u=c.filter(t=>p(t.company));u.sort((t,e)=>{const s=t.createdAt instanceof Date?t.createdAt.getTime():new Date(t.createdAt).getTime();return(e.createdAt instanceof Date?e.createdAt.getTime():new Date(e.createdAt).getTime())-s});const n=[],g=["point speed","invest wise","blueline gadgets"];g.forEach(t=>{const e=u.find(s=>h(s.company)===t&&!n.find(b=>b.id===s.id));e&&n.push(e)}),n.length<3&&g.forEach(t=>{if(n.length>=3)return;const e=u.find(s=>{const i=(s.company||"").toLowerCase().trim();return(t==="point speed"&&i.includes("point")&&i.includes("speed")||t==="invest wise"&&i.includes("invest")&&i.includes("wise")||t==="blueline gadgets"&&i.includes("blue")&&(i.includes("line")||i.includes("gadget")))&&!n.find(x=>x.id===s.id)});e&&n.push(e)}),n.length<3&&u.forEach(t=>{n.length<3&&!n.find(e=>e.id===t.id)&&n.push(t)}),n.length<3&&c.forEach(t=>{n.length<3&&!n.find(e=>e.id===t.id)&&n.push(t)});const f=n.slice(0,3);console.log("Featured testimonials:",f.map(t=>({company:t.company,author:t.authorName}))),m(f)}catch(o){console.error("Error loading testimonials:",o),document.getElementById("testimonials-grid").innerHTML=`
                        <div class="col-span-full text-center py-8">
